...
function processDuplicateFree(list, callback) {
  ...
  const b2Array = [...newList];
  return callback(b2Array);
}
...
function getFullNames(runners) {
  let newArray = [];
  let first = runners.first_name;
  let last = runners.last_name;
    runners.forEach((item, array) => {
      let items = {};
      
      items[item.fullName] = first + ', ' + last;
      newArray.push(items);
    });
    return newArray;
}
...
function firstNamesAllCaps(runners) {
  const newArray = [];
  let fname = runners.first_name;
    runners.map((item) => {
      newArray.push(fname.toUpperCase());
    });
    return newArray;
}
